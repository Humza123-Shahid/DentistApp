...
import { SettingCreate, SettingList, SettingShow, SettingEdit } from './setting';
import { SocialCreate, SocialList, SocialShow, SocialEdit } from './social';
...
const dataProvider1 = {
  getList: async (resource, params) => {
    ...
    const query = {
       ...
       //pagination: JSON.stringify({ page: Number(page), perPage: Number(perPage) }),
  ...
  return (
    <AdminGuard>
      <Admin basename="/admin" layout={MyLayout} dataProvider={dataProvider1} dashboard={Dashboard} >
        ...
        <Resource name="setting" options={{ label: 'Setting' }} list={SettingList} create={SettingCreate} show={SettingShow} edit={SettingEdit} />
        <Resource name="social" options={{ label: 'Social' }} list={SocialList} create={SocialCreate} show={SocialShow} edit={SocialEdit} />
